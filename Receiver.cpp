...
void Receiver::AnalyzeThread(std::string *toAnalyze)
{
    ...
    boost::split(info, *toAnalyze, boost::is_any_of(Sender::getSplit()));//mySender->getSplit()));
    ...
    switch ( choice )
    {
        ...
        case ALLOW_OWNERSHIP:
            //Someone has ownership, determine who and if it is me then I can write/delete...

            userFileOwner = info[1];
            nextRequestID = atoi(info[2].c_str());
            if ( userFileOwner == username )
            {
                hasOwnership = true;
